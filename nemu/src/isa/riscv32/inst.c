...
#define CSRr isa_csr_read
#define CSRw isa_csr_write
...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("0000001 ????? ????? 001 ????? 01100 11", mulh   , R, R(rd) = (word_t) (((int64_t) SEXT(src1, 32) * (int64_t) SEXT(src2, 32)) >> 32));
  ...
  INSTPAT("0000001 ????? ????? 110 ????? 01100 11", rem, R, 
    if (src2 == 0) { R(rd) = src1; } 
    else { R(rd) = (word_t) ((int32_t) src1 % (int32_t) src2); }
  );
  INSTPAT("0000001 ????? ????? 100 ????? 01100 11", div, R, 
    if (src2 == 0) { R(rd) = -1; } 
    else if (src1 == 0x80000000 && src2 == -1) { R(rd) = 0x80000000; } 
    else { R(rd) = (word_t) ((int32_t) src1 / (int32_t) src2); }
  );
  ...
  INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr   , I, is_jalr = 1; vaddr_t t = s->pc + 4; s->dnpc = (src1 + imm) & ~1; R(rd) = t);
  ...
  INSTPAT("0100000 ????? ????? 101 ????? 00100 11", srai   , I, R(rd) = (SEXT(BITS(src1, 31, 31), 1) << (32 - BITS(imm, 4, 0))) | (src1 >> BITS(imm, 4, 0)));
  INSTPAT("??????? ????? ????? 001 ????? 00000 11", lh     , I, R(rd) = SEXT(Mr(src1 + imm, 2), 16));
  INSTPAT("??????? ????? ????? 101 ????? 00000 11", lhu    , I, R(rd) = Mr(src1 + imm, 2));

  INSTPAT("??????? ????? ????? 001 ????? 11100 11", csrrw  , I, word_t t = CSRr(imm); CSRw(imm, src1); R(rd) = t);
  INSTPAT("??????? ????? ????? 010 ????? 11100 11", csrrs  , I, word_t t = CSRr(imm); CSRw(imm, (t | src1)); R(rd) = t);
  ...
  //J
  INSTPAT("??????? ????? ????? ??? ????? 11011 11", jal    , J, is_jal = 1; R(rd) = s->pc + 4; s->dnpc =s->pc + imm);
