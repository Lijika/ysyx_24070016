...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("0000001 ????? ????? 110 ????? 01100 11", rem    , R, R(rd) = ((BITS(src1, 31, 31) ^ BITS(src2, 31, 31)) << 31) | \
  																		(BITS(src1, 31, 31) ? (~src1 + 1) : src1) % (BITS(src2, 31, 31) ? (~src2 + 1) : src2));
//   INSTPAT("0000001 ????? ????? 110 ????? 01100 11", rem    , R, R(rd) = src1 % src2);
  INSTPAT("0000000 ????? ????? 010 ????? 01100 11", blt    , R, R(rd) = CMPs(src1, src2));
  INSTPAT("0000001 ????? ????? 000 ????? 01100 11", mul    , R, R(rd) = src1 * src2);
  INSTPAT("0000001 ????? ????? 100 ????? 01100 11", div    , R, R(rd) = ((BITS(src1, 31, 31) ^ BITS(src2, 31, 31)) << 31) | \
  																		(BITS(src1, 31, 31) ? (~src1 + 1) : src1) / (BITS(src2, 31, 31) ? (~src2 + 1) : src2));
  ...
  INSTPAT("0000001 ????? ????? 001 ????? 01100 11", mulh   , R, R(rd) = ((BITS(src1, 31, 31) ^ BITS(src2, 31, 31)) << 31) | \
  																		(uint32_t)(((uint64_t)(BITS(src1, 31, 31) ? (~src1 + 1) : src1) * (uint64_t)(BITS(src2, 31, 31) ? (~src2 + 1) : src2)) >> 32));
