...
//   INSTPAT("0000001 ????? ????? 110 ????? 01100 11", rem    , R, R(rd) = src1 % src2);
  ...
  INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr   , I, is_jalr = 1; vaddr_t t = s->pc + 4; s->dnpc = (src1 + imm) & ~1; R(rd) = (rd == 0) ? 0 : t);
  ...
  INSTPAT("??????? ????? ????? ??? ????? 11011 11", jal    , J, is_jal = 1; R(rd) = (rd == 0) ? 0 : (s->pc + 4); s->dnpc =s->pc + imm);
